...
fn inspect_message() {
    if is_authorized()
        || !["transfer", "register"].contains(&call::method_name().as_str())
    {
        call::accept_message();
    }
}
...
fn is_authorized() -> bool {
    let checked = STORAGE.read().custodians.contains(&api::caller());
    ic_cdk::println!("Caller from is_authorized: {:?} is_authorized? {:?}", &api::caller().to_string(), checked);
    checked
}
#[query]
fn is_authorizeded(principal: Principal) -> bool {
    let checked = STORAGE.read().custodians.contains(&principal);
    ic_cdk::println!("Caller from is_authorized: {:?} is_authorized? {:?}", &principal.to_string(), checked);
    checked
}
...
fn set_authorized(principal: Principal, authorized: bool) -> Result {
    if authorized {
        ...
        ic_cdk::println!("removed authorized: {:?}", principal.to_string());
        is_authorizeded(principal);
    }
    Ok(())
}
